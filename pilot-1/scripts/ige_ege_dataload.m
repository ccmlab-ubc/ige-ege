...
baseDir= '/Users/hyosubkim/Dropbox/Projects/ige_ege/';
...
subj = {'zidan_ige_ege', 'gregg_ige_ege'};
...
%         S.yf(k,1) = S.hy(k,Nii(k));
        S.xf(k,1) = hX_ep(k);
        S.yf(k,1) = hY_ep(k);
        S.hand_max_dist(k,1) = sqrt((hX_ep(k) - xCenter)^2 + (hY_ep(k) - yCenter)^2);
        ...
        rot_hand_theta(k) = atan2d(sind(hand_angle(k) - tgt_ang(k)), cosd(hand_angle(k) - tgt_ang(k)));
        xy_vec = [hX_ep(k) - xCenter, hY_ep(k) - yCenter];
        tgt_angle = tgt_ang(k) * pi/180;
        rot_angle = -tgt_angle;
        ...
        theta_maxradv(k) = atan2d(sind(S.maxradv_hand_ang(k) - tgt_ang(k)), cosd(S.maxradv_hand_ang(k) - tgt_ang(k)));
...
save('ige_ege_table.mat','T');
save('ige_ege_movefile_table.mat','M');
writetable(T, 'ige_ege.csv', 'Delimiter', ',')
