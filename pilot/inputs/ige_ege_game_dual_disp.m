...
function ige_ege_game_dual_disp(name_prefix, tgt_file_name_prefix, tgt_set)
...
fixed = [];
ccw = 0;
online_fb = tgt_file(:,6);
endpoint_fb = tgt_file(:,7);
clamped_feedback = tgt_file(:,8);
between_blocks = tgt_file(:,9);
targetsize = tgt_file(1,10);
...
% Target location:
if flipped_monitor
    tgtxSubj = xCenterSubj - tgt_dist.*cosd(tgt_ang);
    tgtySubj = yCenterSubj + tgt_dist.*sind(tgt_ang);
else
    tgtxSubj = xCenterSubj + tgt_dist.*cosd(tgt_ang);
    tgtySubj = yCenterSubj - tgt_dist.*sind(tgt_ang);
end
tgtlocSubj = [tgtxSubj tgtySubj];
tgtxInv = xCenterInv + tgt_dist.*cosd(tgt_ang);
tgtyInv = yCenterInv - tgt_dist.*sind(tgt_ang);
tgtlocInv = [tgtxInv tgtyInv];
...
while trial <= maxtrialnum
    ...
    if clamped_feedback(trial,1) == 1
        % Clamped fb location:
        if flipped_monitor
            rcX = xCenterSubj - gain(trial,1)*hand_dist.*cosd(tgt_ang(trial,1) + rotation(trial,1)); % may need to subtract rotation in order to make (+) clamp CCW
            rcY = yCenterSubj + gain(trial,1)*hand_dist.*sind(tgt_ang(trial,1) + rotation(trial,1));
        else
            rcX = xCenterSubj + gain(trial,1)*hand_dist.*cosd(tgt_ang(trial,1) + rotation(trial,1)); % may need to subtract rotation in order to make (+) clamp CCW
            rcY = yCenterSubj - gain(trial,1)*hand_dist.*sind(tgt_ang(trial,1) + rotation(trial,1));
        end
    ...
    elseif gamephase == 1  % Show target
        ...
        % Draw target
        Screen('DrawDots', windowSubj, tgtlocSubj(trial,:), targetsize, green, [], 2);      
        Screen('DrawDots', windowInv, tgtlocInv(trial,:), targetsize, green, [], 2);
        % Draw start circle
        Screen('DrawDots', windowSubj, [xCenterSubj yCenterSubj], startcirclewidth, yellow, [], 2);
        Screen('DrawDots', windowInv, [xCenterInv yCenterInv], startcirclewidth, yellow, [], 2);
    ...
    elseif gamephase == 3  % Endpoint feedback
        ...
        if too_long
            Screen('DrawDots', windowSubj, tgtlocSubj(trial,:), targetsize, red, [], 2);
            Screen('DrawDots', windowInv, tgtlocInv(trial,:), targetsize, red, [], 2);
        else
            Screen('DrawDots', windowSubj, tgtlocSubj(trial,:), targetsize, green, [], 2);
            Screen('DrawDots', windowInv, tgtlocInv(trial,:), targetsize, green, [], 2);
        end
